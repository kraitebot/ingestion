...
### Backups carry the business, not the diagnostics

- [CHANGED] The dump excludes diagnostic stores (`api_request_logs`,
  `api_snapshots`, the two archive tables, dispatcher saturation samples,
  slow queries, failed jobs). Measured on production: 688 MB -> 84 MB. Their
  structure is still dumped, so a restored database boots with them present
  and empty, and every one is regenerated by normal operation.
- [CHANGED] Retention widened from 3 snapshots (~9 hours) to 8 hourly,
  14 daily and 8 weekly — roughly two months — now that a dump is small
  enough for history to be affordable. Corruption noticed on a Monday no
  longer finds Sunday already evicted.
- [ADDED] `kraite.api_request_logs` knobs for how much of an exchange call
  survives: bodies are kept for failures and slow calls, dropped for fast
  successes, with `retain_all_bodies` to restore the old behaviour while
  debugging.
- [WHY] Six consecutive backups failed on 2026-07-28 to Backblaze brownouts
  mid-upload. Bigger multipart chunks (1.81.0) made the upload survivable;
  this makes the target small.
