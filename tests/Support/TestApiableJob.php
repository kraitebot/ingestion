...
/**
 * Flexible test job for BaseApiableJob testing.
 * Configure behavior via step arguments.
 */
final class TestApiableJob extends BaseApiableJob
{
    ...
    public function assignExceptionHandler(): void
    {
        ...
        // Use TestExceptionHandler with behavior controlled by step arguments
        $this->exceptionHandler = new TestExceptionHandler($args);
        $this->exceptionHandler->withAccount($this->account);
    ...
    public function computeApiable()
    {
        ...
        // Throw RequestException (HTTP errors)
        if ($args['throw_request_exception'] ?? false) {
            $this->track('computeApiable:throwing_request_exception', [
                'status' => $args['http_status'] ?? 500,
            ]);

            $response = new Response(
                $args['http_status'] ?? 500,
                $args['response_headers'] ?? [],
                json_encode($args['response_body'] ?? [])
            );

            throw new RequestException(
                $args['exception_message'] ?? 'Request failed',
                new Request('GET', '/test'),
                $response
            );
        }
