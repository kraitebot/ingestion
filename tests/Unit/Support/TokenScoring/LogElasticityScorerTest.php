...
test('zero elasticity produces zero score regardless of correlation', function () {
    expect(LogElasticityScorer::score(0.0, 1.0))->toBe(0.0);
    expect(LogElasticityScorer::score(0.0, 0.5))->toBe(0.0);
});

test('zero correlation produces zero score regardless of elasticity', function () {
    expect(LogElasticityScorer::score(50.0, 0.0))->toBe(0.0);
    expect(LogElasticityScorer::score(-100.0, 0.0))->toBe(0.0);
});

test('score grows monotonically with elasticity at fixed correlation', function () {
...
test('elasticity sign is irrelevant — uses absolute value', function () {
...
test('correlation sign is irrelevant — uses absolute value', function () {
    $positive = LogElasticityScorer::score(15.0, 0.9);
    $negative = LogElasticityScorer::score(15.0, -0.9);
...
test('compresses extreme elasticity so it does not dwarf reasonable candidates', function () {
...
test('log(1+x) base is consistent with natural log expectations', function () {
