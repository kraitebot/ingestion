...
return [
    ...
    'backup' => [
         ...
         * the backups.
         */
        // Lowercase + filesystem-safe so the B2 prefix is a clean
        // path segment (`kraite/...`).
        'name' => 'kraite',

        'source' => [
            'files' => [
                // DB-only backup. The codebase lives in git
                // (`kraitebot/core` + `kraitebot/ingestion`), `.env`
                // is replicated to `~/.credentials`, so a full
                // restore is `git clone` + `composer install` +
                // copy `.env` + import the dump. No upside from
                // zipping vendor/ + node_modules/ into the snapshot.
                'include' => [],
         ...
         */
        // Gzip the SQL dump before it goes into the encrypted zip.
        // The 880MB raw dump compresses to ~80MB, which is what we
        // pay storage for on B2.
        'database_dump_compressor' => Spatie\DbDumper\Compressors\GzipCompressor::class,

        /*
         * If specified, the database dumped file name will contain a timestamp (e.g.: 'Y-m-d-H-i-s').
         */
        // Embed the dump-time stamp in the SQL filename so manual
        // restores can pick the right snapshot without unzipping.
     ...
     */
    'notifications' => [
        // Built-in Mail / Slack / Discord channels disabled — backup
        // events route through the existing `system_health_alert`
        // Pushover canonical via an event listener registered in
        // `app/Providers/EventServiceProvider.php`. Single source of
        // operator alerts; no email-spam vector when a backup hiccups.
        'notifications' => [
    ...
    'cleanup' => [
         ...
         */
        // TieredStrategy: keep last N hourly + N daily + N weekly
        // snapshots (counts in `kraite.backup_tiers`). Default
        // strategy below is left as-is so spatie's config validation
        // doesn't trip — TieredStrategy ignores it.
        'strategy' => App\Support\Backup\TieredStrategy::class,
